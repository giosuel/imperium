...
public class ShotgunGizmo : MonoBehaviour
{
    ...
    private static readonly Color MinEntityDamageColor = new(0.6f, 1.0f, 0.6f);
    private static readonly Color MidEntityDamageColor = new(0.3f, 1.0f, 0.3f);
    private static readonly Color MaxEntityDamageColor = new(0.0f, 0.5f, 0.0f);
    private static readonly Color MinPlayerDamageColor = Color.yellow;
    private static readonly Color MidPlayerDamageColor = new(1f, 0.5f, 0f);
    private static readonly Color MaxPlayerDamageColor = Color.red;
    private static readonly Color SpherecastOriginColor = new(0.5f, 0f, 0.5f);

    private static readonly Color InvalidTargetColor = new(0.3f, 0.3f, 0.3f);
    private static readonly Color TargetObstructedColor = Color.white;

    // Multipler applied to visualization of the shotgun hitbox
    private static readonly Color HitboxColorMultiplier = new(1f, 1f, 1f, 0.35f);
    ...
    private static List<Vector3> GenerateArcPoints(float zOffset, float radius, float degreesArcStart, float degreesArcEnd,
        float maxWidth = Mathf.Infinity)
    {
        const float pi2 = Mathf.PI * 2f;

        var radArcStart = Mathf.Deg2Rad * degreesArcStart;
        var radArcEnd = Mathf.Deg2Rad * degreesArcEnd;

        var radDif = ((radArcEnd - radArcStart) % pi2 + pi2) % pi2;
        ...
        var radCurrent = radArcStart;
        points.Add(new Vector3(Mathf.Sin(radCurrent) * radius, 0, Mathf.Cos(radCurrent) * radius + zOffset));
        do
        {
            radCurrent = Mathf.Min(radCurrent + increment, radArcEnd);
            var xPos = Mathf.Max(Mathf.Min(Mathf.Sin(radCurrent) * radius, maxWidth), -maxWidth);
            var zPos = Mathf.Cos(radCurrent) * radius + zOffset;
            points.Add(new Vector3(xPos, 0, zPos));
    ...
    {
        var renderer = ImpGeometry.CreateLine(
            rayHolder.transform, startColor: lineColor, endColor: lineColor, positions: points.ToArray()
        );
    ...
    private void SetupRendererObject()
    {
        ...
        rayHolder = new GameObject("ImpShotgunRayHolder") { transform = { parent = transform } };
        ...
        spherecastOriginArea = AddLineToRayHolder(
            SpherecastOriginColor * HitboxColorMultiplier, Circle(SpherecastStartOffset, SpherecastRadius)
        );
        ...
        var backSemicircle = GenerateArcPoints(
            SpherecastStartOffset, SpherecastRadius, 270, 90
        );
        var frontSemicircle = GenerateArcPoints(
            SpherecastStartOffset + SpherecastRange, SpherecastRadius, 270, 90
        );
        frontSemicircle.Reverse();
        frontSemicircle.Add(backSemicircle[0]);
        var spherecastRangeShape = backSemicircle.Concat(frontSemicircle).ToList();
        ...
        var radius0 = EntityMidDmgRange;
        var radius1 = SpherecastRadius;
        var dist = Mathf.Abs(SpherecastStartOffset);
        var b = (Mathf.Pow(radius1, 2) - Mathf.Pow(radius0, 2) + Mathf.Pow(dist, 2)) / (2 * dist);
        var a = dist - b;
        var angle = Mathf.Acos(a / radius0);
        var oppAngle = Mathf.Acos(b / radius1);

        var midDmgCircleArc = GenerateArcPoints(
            0, radius0, -180 + angle * Mathf.Rad2Deg, 180 - angle * Mathf.Rad2Deg, SpherecastRadius
        );
        var midDmgOffsetArc = GenerateArcPoints(
            SpherecastStartOffset, radius1, -oppAngle * Mathf.Rad2Deg, oppAngle * Mathf.Rad2Deg, SpherecastRadius
        );
        midDmgOffsetArc.Reverse();
        midDmgOffsetArc.Add(midDmgCircleArc[0]);
        var midDmgArea = midDmgCircleArc.Concat(midDmgOffsetArc).ToList();
        ...
        var killCone = GenerateArcPoints(0, PlayerKillRange, -30, 30);
        ...
        var midDmgCone = GenerateArcPoints(0, PlayerMidDmgRange, -30, 30);
    ...
    private void Update()
    {
        ...
        var hits = Physics.SphereCastNonAlloc(
            ray, SpherecastRadius, colliders, SpherecastRange, 524288, QueryTriggerInteraction.Collide
        );
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            var startPoint = shotgunPosition;
            var endPoint = colliders[i].point;
            ...
            else if (!colliders[i].transform.TryGetComponent<EnemyAICollisionDetect>(out var collisionDetect) ||
                     collisionDetect.onlyCollideWhenGrounded)
            {
            ...
            else if (Physics.Linecast(
                         shotgunPosition, colliders[i].point, out _,
            ...
            {
                var distance = Vector3.Distance(shotgunPosition, colliders[i].point);
    ...
    private static (Vector3, Vector3) GetShotgunPosition(ShotgunItem shotgun)
    {
        ...
        if (shotgun.heldByEnemy && nutcrackerAI)
        {
            if (nutcrackerAI.isInspecting || nutcrackerAI.aimingGun)
                return (shotgun.shotgunRayPoint.position, shotgun.shotgunRayPoint.forward);
