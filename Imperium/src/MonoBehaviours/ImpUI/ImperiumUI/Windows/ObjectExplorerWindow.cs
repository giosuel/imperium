...
internal class ObjectExplorerWindow : BaseWindow
{
    ...
    public void Refresh()
    {
        ...
        var objects = Imperium.ObjectManager.CurrentLevelEntities.Value
            .Where(obj => obj != null)
        ...
        foreach (var (type, component) in objects)
        {
            if (!objectEntries.TryGetValue(component.GetInstanceID(), out var objectEntry))
            {
                ...
                objectEntry.Init(component, parentUI as ImperiumUI);
        ...
        var hazards = objects
            .Where(entry =>
                entry.Key == typeof(ObjectEntryTurret) || entry.Key == typeof(ObjectEntryLandmine) ||
